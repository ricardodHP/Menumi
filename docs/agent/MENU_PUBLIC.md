...
Las tres presentaciones comparten perfil, búsqueda, catálogo visible, carrito, likes, reviews, rating, compartir, disponibilidad, QR, WhatsApp y asistente. La elección de presentación es independiente del tema visual `cuisine_template` y de la apariencia global del menú (`menu_theme`). El dueño puede elegir `Claro` (valor predeterminado y apariencia actual) u `Oscuro`; la elección se aplica a todo el menú sin cambiar la presentación ni la plantilla de cocina. Los cinco temas de cocina actuales son provisionales y sus estilos definitivos quedan pendientes de decisión.
